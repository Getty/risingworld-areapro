	...
	if #cmd >= 1 then
			...
			if #cmd >= 3 then
				...
				if StringUtils:isInteger(cmd[3]) then
					...
					if area ~= nil then
						...
						event.player:sendTextMessage("[#00FF00]Area "..area["areaName"].." successfully filled up with TerrainType "..terraintype);
					else
		...
		elseif cmd[1] == "/blockarea" then
			if #cmd >= 3 then
				...
				if StringUtils:isInteger(cmd[3]) then
					local blockid = tonumber(cmd[3]);
					if area ~= nil then
						world:setBlockDataInArea(area["startChunkpositionX"], area["startChunkpositionY"], area["startChunkpositionZ"], area["startBlockpositionX"], area["startBlockpositionY"], area["startBlockpositionZ"], area["endChunkpositionX"], area["endChunkpositionY"], area["endChunkpositionZ"], area["endBlockpositionX"], area["endBlockpositionY"], area["endBlockpositionZ"], blockid);
						event.player:sendTextMessage("[#00FF00]Area "..area["areaName"].." successfully filled up with BlockID "..blockid);
					else
						event.player:sendTextMessage("[#FF0000]Area "..cmd[2].." not found!");
					end
				else
